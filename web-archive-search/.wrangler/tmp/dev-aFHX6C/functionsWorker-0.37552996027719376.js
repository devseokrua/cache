...
// .wrangler/tmp/bundle-PKjkSq/checked-fetch.js
...
// .wrangler/tmp/pages-TkfD5y/functionsWorker-0.37552996027719376.mjs
...
async function onRequestGet(context) {
  ...
  const [waybackRes, locRes, ukRes, ptRes] = await Promise.allSettled([
    withTimeout(fetchWayback(cleanUrl), 2e4),
    withTimeout(fetchCdx(`https://webarchive.loc.gov/cdx/search/cdx?url=${encodeURIComponent(cleanUrl)}&output=json&fl=timestamp&collapse=timestamp:6&limit=200&filter=statuscode:200`), 8e3),
    withTimeout(fetchCdx(`https://www.webarchive.org.uk/wayback/archive/cdx?url=${encodeURIComponent(cleanUrl)}&output=json&fl=timestamp&collapse=timestamp:6&limit=200&filter=statuscode:200`), 8e3),
    withTimeout(fetchCdx(`https://arquivo.pt/cdx?url=${encodeURIComponent(cleanUrl)}&output=json&fl=timestamp&collapse=timestamp:6&limit=200&statuscode=200`), 8e3)
  ]);
...
__name2(fetchWayback, "fetchWayback");
...
__name(fetchAvailability, "fetchAvailability");
__name2(fetchAvailability, "fetchAvailability");
async function fetchCdx(cdxUrl) {
  let rows;
  try {
    rows = await fetchCdxRaw(cdxUrl);
  } catch {
    return null;
  }
...
__name(fetchCdx, "fetchCdx");
__name2(fetchCdx, "fetchCdx");
...
// .wrangler/tmp/bundle-PKjkSq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PKjkSq/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.37552996027719376.js.map
